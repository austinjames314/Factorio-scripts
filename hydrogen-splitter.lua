on_tick = function (event)
    ...
    Hydrogen = rednet["CW-hydrogen"]
    if Hydrogen == nil then
        Hydrogen = 0
    end

    --'Constants'  *** SET THESE FOR THE SPECIFIC HARDWARE DESIGN ***
    PumpPowerMatrix = {60, 90} -- Thermal power draw at each stage of pump activation.
    PumpSignalMatrix = {"signal-A", "signal-B"} -- Signals to output to activate each level of pumps
    ...
    HydrogenShutdownLevel = 400000 -- 255 seconds rounded up to 400k (1380 * 255 sec. Gives a buffer for shutdown, and startup, plus room for surplus steam during shutdown) 
    HydrogenStartupLevel = 82800 -- 60 seconds (1380 steam for 60 sec = 54000)
    ...
    if Running > 0 then
        ...
        if Hydrogen > HydrogenShutdownLevel then
            output["signal-control-scram"] = 1
        end
    elseif Stopped > 0 then
        --if steam level is low, startup again
        if Hydrogen < HydrogenStartupLevel and CoolantReserve > CoolantCutoff then
